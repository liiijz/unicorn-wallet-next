...
/**
 * PixelAvatar - Minecraft 风格的头像组件
 * 
 * 基于地址生成独特的 8x8 像素头像
 * 灵感来自 https://github.com/drum-n-bass/heads
 */
export default function PixelAvatar({ address, size = 64, className = "" }: PixelAvatarProps) {
...
function drawAvatar(ctx: CanvasRenderingContext2D, address: string) {
...
function drawLayer(
  ...
  data: string,
  color: { hue: number; saturation: number; lightness: number; variation: number },
  seed: string
) {
...
function drawPixel(
  ...
  const rnd1 = pick(seed, 1);
  const rnd2 = pick(seed, 2);
  const variation = (x + rnd1 + y + rnd2) * color.variation;

  const hsl = `hsl(${color.hue + variation}, ${color.saturation}%, ${color.lightness}%)`;
  ctx.beginPath();
  ctx.rect(x, y, 1, 1);
  ctx.fillStyle = hsl;
  ctx.fill();
...
function pick(seed: string, pos: number, length = 3): number {
  return hex2dec(seed.substring(pos, pos + length));
}
